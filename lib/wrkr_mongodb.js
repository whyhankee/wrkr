...
WrkrMongodb.prototype.publish = function publish(events, done) {
  ...
  function sendEvent(event, cb) {
    if (!event.name) throw new Error('publish: missing event.name');

    async.waterfall([getQueuesForEvent, sendQueueItems], cb);
    return;

    function getQueuesForEvent(cb) {
      self._queuesForEvent(event.name, cb);
    }
    function sendQueueItems(queues, cb) {
    ...
    self.QueueItems.findOneAndUpdate(searchSpec, updateSpec, sortOptions, function (err, event) {
      ...
      function dispatch(cb) {
        ...
        self.wrkr._dispatch(eventObj, function (err) {
          debug('dispatched', eventObj.name);
          return cb(err, event);
        });
      }
      ...
      function onDone(err /*, savedEvent */) {
        ...
        // Report processing errors to the Wrkr interface
        if (err) self.wrkr._reportError(err);
