...
WrkrMongodb.prototype.start = function (wrkr, opt, done) {
  ...
  this.dbConnectStr = `mongodb://${dbUserPswd}${this.opt.host}:${this.opt.port}/${this.opt.name}`;

  // Connect
  debug('starting wrkr_mongodb :'+this.dbConnectStr);
  this.db = mongoose.createConnection();
  this.db.open(this.dbConnectStr, this.opt.dbOpt, (err) => {
...
WrkrMongodb.prototype.getQueueItems = function getQueueItems(filter, done) {
  ...
  this.QueueItems.find(mongoFilter, {}, {created: 1}, done);
};
 ...
 ********************************************************************/
 ...
 WrkrMongodb.prototype.archiveArchive = function archiveArchive(event, done) {
 ...
 WrkrMongodb.prototype.archiveDelete = function archiveDelete(event, done) {
   debug(`deleting processed qitem ${event.name} ${event.tid}`);
