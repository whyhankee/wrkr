...
Wrkr.prototype.getQueueItems = function getQueueItems(filter, done) {
  ...
  // at least two options, name should be one of them
  if (Object.keys(filterSpec).length < 2 || !filterSpec.name) {
...
Wrkr.prototype.deleteQueueItems = function deleteQueueItems(filter, done) {
  ...
  // at least two options, tid should be one of them
  if (Object.keys(filterSpec).length < 2 || !filterSpec.tid) {
    debug('invalid search spec ', filterSpec);
