...
// Wrkr
//
//  routing     = [array of objects]
//    .eventName
//    .queues
// TODO: Store subscriptions in Store
//
...
//
Wrkr.prototype.emit = function emit(events, done) {
  var eventList = Array.isArray(events) ? events : [events];
  debug('emit', eventList);
  this.store.emit(eventList, done);
};
...
Wrkr.prototype._dispatch = function _dispatch(event, done) {
  ...
  if (!self.eventHandlers[event.eventName]) {
    debug('noImplementedYet', event.eventName);
    return done();
  }
  self.eventHandlers[event.eventName](event, function (err) {
