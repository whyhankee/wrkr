...
Wrkr.prototype.stop = function stop(done) {
  ...
  function waitProcessing(cb) {
    ...
    debug('Wait for in-process qitem ..');
...
Wrkr.prototype._dispatch = function _dispatch(event, done) {
  ...
  function onDispatchDone(err) {
    if (err) {
      debug(`error received for ${event.name} (${event.tid}): `+err.toString());
      self._reportError(err, event);
    }

    debug(`ack event ${event.name} (${event.tid})`, self.archiveMethod);

    if (self.archiveMethod === 'markDone') {
      // should already have been done by polling
      return onArchiveDone(null);
      ...
      return self.backend.deleteQueueItems(deleteSpec, onArchiveDone);
    } else {
...
Wrkr.prototype._reportError = function _reportError(err, event) {
  var handled = this.emit('error', err, event) === true;
  if (!handled) {
